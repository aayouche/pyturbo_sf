...
Figures \ref{fig:swot}–\ref{fig:dycoms} demonstrate PyTurbo_SF applied across scientific domains, illustrating the package's versatility for both satellite observations and numerical simulations.

**Figure 1** presents an analysis of NASA's Surface Water and Ocean Topography (SWOT) satellite altimetry data [@Morrow2019] in the Gulf Stream region. The upper panel displays the zonal geostrophic velocity field ($u_g$, m/s) derived from sea surface height gradients, mapped onto a local Cartesian coordinate system where $X$ and $Y$ represent eastward and northward distances in kilometers. The two diagonal swaths correspond to SWOT's twin Ka-band radar interferometer tracks. The lower panel shows isotropically-averaged structure functions computed using `get_isotropic_sf_2d()`: the third-order longitudinal SF divided by separation distance ($\mathrm{SF}_{LLL}/r$, blue) computed with `fun='longitudinal'` and `order=3`, and the advective SF ($\mathrm{ASF}_V$, red) computed with `fun='advective'`. Both quantities have units of energy dissipation rate (m$^2$\,s$^{-3}$). The shaded regions indicate 95% bootstrap confidence intervals. Physically, these third-order SFs are proportional to the spectral energy flux: positive values indicate forward cascade (energy transfer to smaller scales) while negative values indicate inverse cascade (energy transfer to larger scales). The sign change near 100 km separation distance reveals the transition between cascade regimes characteristic of quasi-geostrophic ocean turbulence.

**Figures 2 and 3** analyze output from the Coastal and Regional Ocean COmmunity model (CROCO) [@Shchepetkin2005], a high-resolution regional ocean simulation. Figure 2 presents a four-panel analysis: the left panels show the input fields—absolute vorticity $\omega = \zeta_z + f$ (vertical component of relative vorticity plus Coriolis parameter, upper left) and zonal velocity $u$ (lower left)—over a 150 km × 250 km domain. The right panels compare different SF diagnostics. The upper-right panel displays third-order and advective SFs related to energy flux: the velocity advective SF ($\mathrm{ASF}_V$, solid red) and $\mathrm{SF}_{LLL}/r$ (dashed red) on the left axis (m$^2$\,s$^{-3}$), alongside the negative vorticity advective SF ($-\mathrm{ASF}_\omega$, solid blue) and $-\mathrm{SF}_{L\omega\omega}/r$ (dashed blue) on the right axis (s$^{-3}$). The lower-right panel shows traditional second-order SFs: longitudinal ($\mathrm{SF}_{LL}$, solid red) and transverse ($\mathrm{SF}_{TT}$, dashed red) velocity SFs on the left axis (m$^2$\,s$^{-2}$), and the scalar vorticity SF ($\mathrm{SF}_{\omega\omega}$, blue with shading) on the right axis (s$^{-2}$). Shaded regions represent 95% bootstrap confidence intervals. All isotropic SFs were computed using `get_isotropic_sf_2d()` with appropriate `fun` and `order` parameters.

**Figure 3** displays 2D (non-isotropic) structure functions computed using `bin_sf_2d()`, showing how SF values vary with separation distance in both $x$ (zonal) and $y$ (meridional) directions. From left to right: the advective SF ($\mathrm{ASF}_V$, m$^2$\,s$^{-3}$), the third-order longitudinal SF ($\mathrm{SF}_{LLL}$, m$^3$\,s$^{-3}$), and the second-order longitudinal SF ($\mathrm{SF}_{LL}$, m$^2$\,s$^{-2}$). The axes show separation distances from $10^3$ to $10^5$ m on logarithmic scales. Anisotropic features are visible: the SFs are not perfectly symmetric about the diagonal, indicating that turbulent energy transfer differs between zonal and meridional directions, likely reflecting the influence of the background flow and mesoscale eddy structures. White regions at large separations indicate insufficient data pairs for reliable statistics.

**Figure 4** demonstrates 3D capabilities using output from the DYCOMS-II (Second Dynamics and Chemistry of Marine Stratocumulus) large eddy simulation [@Stevens2005], which models the stratocumulus-topped marine boundary layer. The figure shows the second-order longitudinal SF ($\mathrm{SF}_{LL}$, m$^2$\,s$^{-2}$) computed using `bin_sf_3d()` with `fun='longitudinal'` and `order=2`. The three axes represent separation distances in meters: $r_x$ (horizontal, 0–3500 m), $r_y$ (horizontal, 0–3500 m), and $r_z$ (vertical, 0–45000 m, noting the different scale). The color scale ranges from ~0.3 to ~0.9 m$^2$\,s$^{-2}$. The SF increases with separation distance in all directions, as expected for turbulent flows. The approximately symmetric structure in the horizontal ($r_x$–$r_y$) plane indicates isotropic horizontal turbulence, while the different scaling in the vertical direction reflects the anisotropy imposed by stable stratification at the boundary layer top. This 3D visualization enables assessment of turbulence isotropy and identification of characteristic length scales in atmospheric boundary layer flows.
