...
![Three-dimensional structure function analysis of DYCOMS-II (Second Dynamics and Chemistry of Marine Stratocumulus) large eddy simulation output [@Stevens2005], modeling the stratocumulus-topped marine boundary layer. The figure displays the second-order longitudinal SF ($\mathrm{SF}_{LL}$, m$^2$\ s$^{-2}$) computed using `bin_sf_3d()` with `fun='longitudinal'` and `order=2`. Axes represent separation distances: $r_x$ and $r_y$ (horizontal, 0–3500 m) and $r_z$ (vertical, 0–45000 m). Color scale ranges from ~0.3 to ~0.9 m$^2$\ s$^{-2}$. The SF increases with separation distance as expected for turbulent flows. The approximately symmetric structure in the horizontal ($r_x$–$r_y$) plane indicates isotropic horizontal turbulence, while different scaling in the vertical direction reflects anisotropy imposed by stable stratification at the boundary layer top.\label{fig:dycoms}](figs/figure4.png)

# Related work and scientific impact

PyTurbo_SF represents a significant advancement by uniquely combining comprehensive function types, adaptive bootstrap methodology, and optimized algorithms — no existing software delivers this combination of statistical rigor, efficiency, and breadth.

FluidSF [@Wagner2025] is a related Python package for structure function calculations, supporting 1D/2D/3D data and core structure function types including longitudinal, transverse, scalar, and advective functions. At the time of this writing, FluidSF provides second- and third-order structure functions with a simplified isotropic averaging approach and outputs results as NumPy [@Harris2020] arrays. It offers a velocity-scalar blended cross-term type and a single transverse decomposition in 3D.

PyTurbo_SF is, to our knowledge, the first open-source package to combine arbitrary-order structure function computation, adaptive bootstrap uncertainty quantification with automatic convergence monitoring, and parallel processing in a single framework. It provides extensive cross-term structure function types (longitudinal-transverse, longitudinal-scalar, transverse-scalar, and scalar-scalar), full 3D transverse decomposition across all three coordinate planes (ij, ik, jk), conditional structure functions, exact spherical and polar binning for isotropic averaging, and spectral energy flux estimation via Bessel transform — all accelerated by power-of-2 spacing strategies and joblib [@Joblib2020]-based parallelization. Results are returned as richly annotated xarray [@Hoyer2017] Datasets with embedded metadata, convergence diagnostics, and quality masks.

PyTurbo_SF's primary contributions are: (1) rigorous uncertainty quantification through adaptive bootstrap resampling with automatic convergence monitoring, enabling robust statistical comparisons and hypothesis testing; (2) support for arbitrary-order structure functions essential for intermittency analysis; and (3) computational efficiency through parallelization and power-of-2 spacing strategies enabling analysis of large datasets.

The package directly supports recent theoretical developments, particularly advective structure functions providing direct energy flux measurements [@Pearson2021] and spectral flux estimation methodologies [@Pearson2024]. These reveal energy pathways traditional approaches cannot capture, and the consistent methodology enables comparative studies across domains — from satellite altimetry to large eddy simulations — previously impossible due to software limitations.
